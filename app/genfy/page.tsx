...
import { Image as ImageIcon, Sparkles, Download, Loader2, Settings2, Sliders, Cpu } from "lucide-react";
...
export default function GenfyPage() {
  ...
  const [result, setResult] = useState<{ image_base64: string; seed: number } | null>(null);
  ...
  // Ambil daftar model dari backend saat startup
  useEffect(() => {
    fetch("http://localhost:8000/models")
      .then(res => res.json())
      .then(data => {
        setModelList(data);
        if (data.length > 0) setModelName(data[0]); // Set model pertama sebagai default
      })
      .catch(err => setError("Gagal mengambil daftar model. Pastikan backend jalan."));
  }, []);
    ...
    <div className="min-h-screen bg-slate-950 text-white p-4 md:p-8 font-sans">
      <header className="max-w-6xl mx-auto mb-10 text-center">
        <h1 className="text-6xl font-black tracking-tighter bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-500 bg-clip-text text-transparent mb-2">
          GENFY <span className="text-sm font-mono text-slate-500 border border-slate-800 px-2 py-1 rounded">PRO</span>
        </h1>
        <p className="text-slate-400 font-medium tracking-wide">Single File Checkpoint Studio</p>
      </header>
            ...
            <form onSubmit={handleGenerate} className="space-y-6">
              ...
              <div>
                <label className="flex items-center gap-2 text-sm font-semibold mb-3 text-slate-300">
                  <Cpu size={16} className="text-indigo-400" /> Choose Checkpoint (.safetensors)
                </label>
                <select 
                  value={modelName}
              ...
              {/* Input Prompt, Settings & Button (Sama seperti sebelumnya) */}
              {/* ... (Copy bagian Prompt, Sliders, dan Button dari kode page.tsx sebelumnya) ... */}
              
              <div>
                <label className="flex items-center gap-2 text-sm font-semibold mb-3 text-slate-300">
                  <Sparkles size={16} className="text-purple-400" /> Deskripsi Gambar (Prompt)
                </label>
                <textarea
                  value={prompt}
              ...
              <div className="pt-4 border-t border-slate-800 space-y-5">
                <div className="space-y-2">
                  <div className="flex justify-between text-xs">
                    <span className="text-slate-400 font-medium">Inference Steps: <span className="text-purple-400">{steps}</span></span>
                  </div>
                  <input type="range" min="1" max="100" value={steps} onChange={(e) => setSteps(parseInt(e.target.value))} className="w-full h-1.5 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-purple-500" />
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between text-xs">
                    <span className="text-slate-400 font-medium">CFG Scale: <span className="text-pink-400">{cfg}</span></span>
                  </div>
                  <input type="range" min="1" max="20" step="0.5" value={cfg} onChange={(e) => setCfg(parseFloat(e.target.value))} className="w-full h-1.5 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-pink-500" />
                </div>
              </div>
              ...
              >
                {loading ? <Loader2 className="animate-spin" /> : <Sliders size={18} />}
        ...
        <section className="lg:col-span-7 bg-slate-900/30 border border-slate-800 rounded-3xl min-h-[500px] flex flex-col items-center justify-center relative overflow-hidden backdrop-blur-sm">
           ...
           {result ? (
            <div className="w-full p-4">
              <img src={result.image_base64} alt="AI Result" className="w-full rounded-2xl shadow-2xl border border-slate-700" />
              <div className="mt-4 flex items-center justify-between px-2">
                <p className="text-xs text-slate-400 italic">Seed: {result.seed} • Model: {modelName}</p>
                <button onClick={downloadImage} className="p-2 bg-slate-800 rounded-full hover:bg-slate-700 transition-colors"><Download size={18} /></button>
              </div>
            </div>
          ) : (
            <div className="text-center group p-10">
              <ImageIcon className="text-slate-600 mx-auto mb-6" size={48} />
              <h3 className="text-slate-400 font-medium text-lg italic">The canvas is empty...</h3>
            </div>
          )}
          {loading && (
            <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center z-50">
               <div className="text-center">
                ...
                <p className="text-purple-400 font-bold text-xl animate-pulse">AI is Creating</p>
                <p className="text-slate-500 text-xs mt-3 italic">Menyiapkan checkpoint file bergiga-giga...</p>
              </div>
            </div>
          )}
        </section>
